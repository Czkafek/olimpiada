	...
	if (stations[n].first + k < m)
		...
		while (true) {
			...
			while (lastStation + 1 <= n && stations[lastStation + 1].first <= stations[curStation].first + k) {
				...
				if (stations[lastStation].second < stations[minIdx].second)
					minIdx = lastStation;
			}
			...
			if (stations[minIdx].second <= stations[curStation].second) {
				ll a = stations[minIdx].first - stations[curStation].first - tank;
				if (a < 0)
					tank = a * -1;
				else {
